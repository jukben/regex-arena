...
class FinalEvaluationOutput(BaseModel):
    final_regex: str = Field(..., description="The final regex pattern submitted by the Code Implementation Agent")
    score: int = Field(..., ge=0, le=100, description="The score between 0-100 indicating regex performance")
    false_negatives: List[str] = Field(..., description="Valid inputs that were not matched")
    false_positives: List[str] = Field(..., description="Invalid inputs that were incorrectly matched")
...
def run_regex_crew(problem=None):
    ...
    if problem:
        ...
        generate_test_suite_round1.description = generate_test_suite_round1.description.replace(
